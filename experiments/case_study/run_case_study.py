"""One user, traced across α.

The aggregate numbers hide what steering actually does to a single decision, so
this records the top-k next-token distribution at the answer position for one
LaMP-2 user as α grows from 0 through the operating point to over-steering.

    python experiments/case_study/run_case_study.py --user_idx 3 --variant fact
"""
...
def first_token_topk(
    ...
    """Top-k tokens at the answer position, plus the greedy continuation."""
...
def main():
    ...
    ap.add_argument("--user_idx", type=int, default=1,
                    help="Index into the deduplicated unique-user list.")
    ...
    if args.variant == "fact":
        facts = FactExtractor(model=model, tokenizer=tokenizer, task=args.task) \
            .extract_facts(s["behavior_profile_text"])
    ...
    for alpha in args.alphas:
        ...
        rows.append({"alpha": alpha, "prediction": pred, "match_gold": match,
                     "topk": topk})

    out_dir = ROOT / args.output_dir
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f"case_{args.task}_user{args.user_idx:03d}_{args.variant}.json"
